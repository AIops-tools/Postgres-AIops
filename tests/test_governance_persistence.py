...
throwaway home (``POSTGRES_AIOPS_HOME``) and assert that the rows compliance
...
def gov_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("POSTGRES_AUDIT_APPROVED_BY", raising=False)
...
def test_high_risk_allowed_with_named_approver(gov_home, monkeypatch):
    monkeypatch.setenv("POSTGRES_AUDIT_APPROVED_BY", "dba-alice")
...
@pytest.mark.unit
def test_real_write_tool_persists_priorstate_undo(gov_home, monkeypatch, fake_pg):
    """End-to-end through a REAL product write tool: drop_index must capture
    pg_get_indexdef before dropping and persist the recreate-undo on disk."""
    monkeypatch.setenv("POSTGRES_AUDIT_APPROVED_BY", "dba-alice")
    indexdef = "CREATE INDEX idx_orders_ts ON public.orders USING btree (ts)"
    ...
    result = gov.drop_index(name="idx_orders_ts")
    assert result["priorState"]["indexdef"] == indexdef
    assert result.get("_undo_id")

    undo = _rows(gov_home / "undo.db", "undo_log")
    assert len(undo) == 1
    assert indexdef in undo[0]["undo_params"]
