...
def test_governed_write_is_refused_in_read_only(read_only):
    """A write is refused before it can reach the cluster — from any caller."""
    calls = []

    @governed_tool(risk_level="high")
    def terminate_backend(pid: int = 4242) -> dict:
        calls.append(pid)
...
def test_governed_read_still_works_in_read_only(read_only):
    ...
    def table_sizes(limit: int = 20) -> dict:
        return {"tables": [{"table": "orders"}], "returned": 1}

    assert table_sizes() == {"tables": [{"table": "orders"}], "returned": 1}


@pytest.mark.unit
def test_medium_risk_write_is_also_refused(read_only):
    """Only 'low' is a read — medium (e.g. run_vacuum, undo_apply) is refused too."""

    @governed_tool(risk_level="medium")
    def run_vacuum(table: str = "public.orders") -> dict:
...
def test_apply_read_only_unregisters_write_tools(monkeypatch):
    ...
    try:
        ...
        # A destructive one, a DDL one, and the mid-tier maintenance writes.
        for name in ("terminate_backend", "drop_index", "run_vacuum", "update_setting"):
            assert name in dropped, f"{name} is a write and must be unregistered"
        for name in ("table_bloat", "top_queries", "slow_query_rca"):
            assert name not in dropped, f"{name} is a read and must survive"
        ...
        assert "table_bloat" in remaining, "reads must still be exposed"
    finally:
        registry.clear()
