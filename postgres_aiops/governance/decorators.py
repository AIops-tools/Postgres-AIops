"""The ``@governed_tool`` decorator — mandatory wrapper for all PostgreSQL MCP tool functions.

Responsibilities:
  1. Pre-check: evaluate policy rules (deny, maintenance window)
  2. Execute: run the actual tool function
  3. Post-log: write audit record to ``~/.postgres-aiops/audit.db``
...
from postgres_aiops.governance.audit import detect_agent, get_engine
from postgres_aiops.governance.budget import BudgetExceeded, get_budget
from postgres_aiops.governance.outcome import (
    clear_prior_state,
    is_unknown,
    take_prior_state,
)
from postgres_aiops.governance.patterns import PatternMatch, get_pattern_engine
from postgres_aiops.governance.policy import PolicyResult, get_policy_engine
from postgres_aiops.governance.readonly import is_read_only, read_only_denial
from postgres_aiops.governance.sanitize import sanitize

_log = logging.getLogger("postgres-aiops.decorators")
...
def governed_tool(
    ...
    *,
    ...
    """Decorator for all PostgreSQL MCP tool functions.
    ...
    Args:
        ...
        undo: Optional callable ``(params, result) -> dict | None`` returning an
            ...
            successful call the inverse is recorded to ~/.postgres-aiops/undo.db and the
...
class _CallState:
    ...
    def __init__(
        ...
        self.rationale = os.environ.get("POSTGRES_AUDIT_RATIONALE", "")
        self.approved_by = os.environ.get("POSTGRES_AUDIT_APPROVED_BY", "")
        self.risk_tier = ""
...
def _pre_check(state: _CallState) -> None:
    ...
    # review) are denied when none was recorded (POSTGRES_AUDIT_APPROVED_BY).
    ...
    if tier.requires_approver and not state.approved_by and not state.dry_run:
        reason = (
            ...
            f"Set POSTGRES_AUDIT_APPROVED_BY to the authorizing human (and "
            f"POSTGRES_AUDIT_RATIONALE to why) before retrying."
        )
        if tier.reason:
...
def _record_undo(state: _CallState, result: Any) -> None:
    ...
    * **Undetermined** — the response was lost (see
      :mod:`postgres_aiops.governance.outcome`). The change may be live, so an
...
def _finalize(state: _CallState) -> None:
    ...
    # cancellation (cancelling mid-flight PostgreSQL calls is worse).
...
def _infer_skill(func: Any) -> str:
    ...
    ``postgres_aiops.ops.jobs`` → ``postgres-aiops``
    ``mcp_server.server`` → ``postgres-aiops`` (the only consumer here).
    """
    module = getattr(func, "__module__", "") or ""
    if module.startswith("postgres_aiops") or module.startswith("mcp_server"):
        return "postgres-aiops"
    return "unknown"
