"""Environment and connectivity diagnostics for Postgres AIops."""

from __future__ import annotations

from rich.console import Console

from postgres_aiops.config import CONFIG_FILE, ENV_FILE, load_config
from postgres_aiops.secretstore import SECRETS_FILE, check_permissions, has_store
...
def run_doctor(skip_auth: bool = False) -> int:
    ...
    if not CONFIG_FILE.exists():
        ...
        _console.print("[yellow]  Run 'postgres-aiops init' to set up your first target.[/]")
        return 1
    ...
    if has_store():
        ...
        _console.print(
            ...
            f"'postgres-aiops secret migrate'.[/]"
        )
    else:
        _console.print(
            "[yellow]! No secret store yet. Run 'postgres-aiops init' to set up "
    ...
    for target in config.targets:
        try:
            ...
            row = conn.query_one("SELECT version() AS version") or {}
