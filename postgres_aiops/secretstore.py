"""Encrypted secret storage for postgres-aiops.

Secrets (PostgreSQL role passwords) are NEVER written to disk in plaintext. They
live in ``~/.postgres-aiops/secrets.enc`` encrypted with Fernet (AES-128-CBC +
...
The master password is resolved (in order) from:
  ...
  1. the env var ``POSTGRES_AIOPS_MASTER_PASSWORD`` (for non-interactive use:
...
def migrate_legacy_env(prefix: str, suffix: str, password: str | None = None) -> list[str]:
    ...
    Legacy keys look like ``PG_<TARGET>_PASSWORD``; ``prefix='PG_'`` and
