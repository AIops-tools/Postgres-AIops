"""``postgres-aiops secret`` — manage the encrypted credential store.

Secrets (PostgreSQL role passwords) are stored in ``~/.postgres-aiops/secrets.enc``
...
from postgres_aiops.config import SECRET_ENV_PREFIX, SECRET_ENV_SUFFIX
from postgres_aiops.secretstore import (
    SECRETS_FILE,
    SecretStore,
    check_permissions,
    migrate_legacy_env,
...
def secret_list() -> None:
    ...
    names = store.names()
    if not names:
        console.print("[yellow]No secrets stored yet. Add one: postgres-aiops secret set <name>[/]")
        return
...
def secret_rotate_password() -> None:
    ...
    console.print("[green]✓ Master password rotated. Update POSTGRES_AIOPS_MASTER_PASSWORD.[/]")
